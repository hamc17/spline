...
import { getColorFromOperationType, getIconFromOperationType } from '../reducers/execution-plan.reducer';
...
export class ExecutionPlanEffects {
    ...
    @Effect()
    getExecutionPlan$: Observable<Action> = this.actions$.pipe(
        ofType(ExecutionPlanAction.ExecutionPlanActionTypes.EXECUTION_PLAN_GET),
        mergeMap((action: any) => {
            return this.getExecutedLogicalPlan(action.payload)
        }),
        map(res => {
            return new ExecutionPlanAction.GetSuccess(res)
        })
    )

    private getExecutedLogicalPlan(executionPlanId: string): Observable<ExecutedLogicalPlanVM> {
        return this.store.select('config').pipe(
            mergeMap(res => {
                if (res == null) {
                    return empty()
                }
                this.executionPlanControllerService.rootUrl = res.apiUrl
                return this.executionPlanControllerService.lineageUsingGETResponse(executionPlanId).pipe(
                    map(response => {
                        return this.toLogicalPlanView(response)
                    }),
    ...
    private toLogicalPlanView = (executedLogicalPlanHttpResponse: StrictHttpResponse<ExecutedLogicalPlan>): ExecutedLogicalPlanVM => {
        ...
        _.each(executedLogicalPlanHttpResponse.body.plan.nodes, (node: Operation) => {
            ...
            cytoscapeOperation.color = getColorFromOperationType(node.name)
            cytoscapeOperation.icon = getIconFromOperationType(node.name)
