...
export function attributeReducer(state: any, action: Action) {
    switch (action.type) {
        case AttributesAction.AttributesActionTypes.ATTRIBUTES_GET: return { ...state, ...buildAttributeGraph(action.payload) }
        default: return null
    }
}
...
function buildAttributeGraph(attribute: AttributeVM, parentAttribute?: AttributeVM, graph?: CytoscapeGraphVM, depth: number = 0): CytoscapeGraphVM {
    ...
    if (attribute.name) {
        let node: GraphNode = { data: { id: attribute.name + graphDepth, name: attribute.name + ' : ' + getAttributeType(attribute), icon: 0xf111, color: getAttributeColor(attribute) } }
        graph.nodes.push(node)
        if (parentAttribute != null) {
            let edge = { data: { source: parentAttribute.name + (graphDepth - 1), target: attribute.name + graphDepth, id: attribute.name + parentAttribute.name } }
            graph.edges.push(edge)
        }
    }
    let childrenAttributes = getChildrenAttributes(attribute)
    _.each(childrenAttributes, item => {
        buildAttributeGraph(item, attribute, graph, graphDepth)
    })
    return graph
}
...
function getAttributeColor(attribute: AttributeVM): string {
    switch (attribute.dataType._type) {
        ...
        case AttributeType.Array: return '#e39255'
