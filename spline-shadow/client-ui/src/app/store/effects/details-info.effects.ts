...
export class DetailsInfoEffects {
    ...
    getDetailsInfo$: Observable<Action> = this.actions$.pipe(
        ...
        flatMap((action: any) => {
            return this.getDetailsInfo(action.payload)
        }),
        map(res => {
            return new DetailsInfoAction.GetSuccess(res)
        })
    )
    ...
    private getDetailsInfo(nodeId: string): Observable<OperationDetailsVM> {
        return this.store.select('config').pipe(
            mergeMap(res => {
                ...
                this.operationDetailsControllerService.rootUrl = res.apiUrl
                return this.operationDetailsControllerService.operationUsingGETResponse(nodeId).pipe(
                    map(response => {
                        return this.toOperationDetailsView(response)
                    }),
    ...
    private toOperationDetailsView = (operationDetailsVMHttpResponse: StrictHttpResponse<OperationDetails>): OperationDetailsVM => {
        ...
        _.each(operationDetailsVMHttpResponse.body.schemas, (attributeRefArray: Array<AttributeRef>) => {
            const attributes = _.map(attributeRefArray, attRef => {
                return this.getAttribute(attRef.dataTypeKey, operationDetailsVMHttpResponse.body.dataTypes, attributeRefArray, attRef.name)
            })
    ...
    private getDataType(dataTypes: Array<DataType>, dataTypeId: string): GenericDataTypeVM {
        return _.find(dataTypes, function (dt: GenericDataTypeVM) {
            return dt._key == dataTypeId
        })
    }
