...
import { DetailsInfoEffects } from './store/effects/details-info.effects';
import { ExecutionPlanEffects } from './store/effects/execution-plan.effects';
...
export function initializeApp(store: Store<AppState>) {
  return () => {
    ...
    store.select((state: AppState) => state).toPromise().then(res => {
